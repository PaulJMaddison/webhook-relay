...
impl ReplayQuery {
    fn retry_count(&self) -> Result<u8, AppError> {
        let retries = self
            .retries
            .unwrap_or((REPLAY_MAX_ATTEMPTS.saturating_sub(1)) as u8);
        if usize::from(retries) >= REPLAY_MAX_ATTEMPTS {
            return Err(AppError::validation(format!(
                "retries must be between 0 and {}",
                REPLAY_MAX_ATTEMPTS.saturating_sub(1)
            )));
        }
...
async fn replay_event(
    ...
    let max_attempts = usize::from(retries) + 1;
    let method = event
        .method
    ...
    for attempt_idx in 0..max_attempts {
        ...
        state.store.create_delivery_attempt(attempt.clone()).await?;

        attempts.push(ReplayAttemptResponse {
...
async fn reset_event(
    ...
    Ok((
        ...
        Json(ReplayResponse {
            event_id: id,
            attempts: Vec::new(),
            final_status: EventStatus::Received,
        }),
    ))
}
...
mod tests {
    ...
    async fn ingest_with_idempotency_key_duplicate_returns_existing_event_id() {
        ...
        let stored = store.events.read().await;
        assert_eq!(stored[0].event.source, "github");
        assert_eq!(stored[0].event.body, b"{\"first\":true}".to_vec());
        assert_eq!(
            stored[0].event.body_size_bytes as usize,
            stored[0].event.body.len()
        );
